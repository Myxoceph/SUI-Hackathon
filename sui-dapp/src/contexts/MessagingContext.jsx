...
export const MessagingProvider = ({ children }) => {
  ...
  const fetchOnChainMessages = useCallback(
  ...
  const sendMessage = useCallback(
    async (recipientAddress, content) => {
      if (!userAddress) {
  ...
  const updateConversation = useCallback(
    (recipientAddress, lastMessage) => {
      setConversations(prev => {
        const existing = prev.find(
          c =>
            c.participants?.includes(recipientAddress) &&
            c.participants?.includes(userAddress)
        )

        if (existing) {
          return prev
            .map(c =>
              c === existing ? { ...c, lastMessage, updatedAt: Date.now() } : c
            )
