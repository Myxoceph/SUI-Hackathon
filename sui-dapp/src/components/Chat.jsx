...
const Chat = ({ recipientAddress, recipientName = null, className }) => {
  ...
  const handleSend = async e => {
    ...
    if (result) {
      console.warn('Message sent:', result.digest)
    }
  }
    ...
    return (
      ...
      >
        ...
        <h3 className="text-lg font-semibold mb-2">
          {t('components.chat.walletRequired')}
        </h3>
      ...
      <div className="flex items-center gap-3 p-4 border-b border-border bg-card">
        <div
          className={cn(
            ...
            getAddressColor(recipientAddress)
          )}
        >
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-muted/20">
        {loading && messages.length === 0 ? (
          ...
          messages.map((message, index) => {
            ...
            const showAvatar =
              !isOwn &&
              (index === 0 || messages[index - 1]?.sender !== senderAddress)
