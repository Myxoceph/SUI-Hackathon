    ...
    <>
      <div className="group border border-border bg-card hover:border-primary/50 transition-colors p-6 space-y-4 flex flex-col">
        ...
        <div className="flex justify-between items-start gap-2">
          <Badge
            variant="outline"
            className={`rounded-none font-mono text-xs ${getStatusColor()}`}
          >
        ...
        {status === JOB_STATUS.ASSIGNED && (
          <div className="text-xs space-y-1 bg-muted/50 p-2 border border-border">
            <p className="font-mono text-muted-foreground">
              {t('components.jobCard.completionStatus')}:
            </p>
            <div className="flex gap-4">
              <span
                className={
                  ownerConfirmed ? 'text-green-500' : 'text-muted-foreground'
                }
              >
              ...
              <span
                className={
                  workerConfirmed ? 'text-green-500' : 'text-muted-foreground'
                }
              >
        ...
        <div className="pt-4 border-t border-border flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            {isAssignedWorker && (
              <Badge
                variant="secondary"
                className="text-xs bg-blue-500/20 text-blue-500"
              >
                {t('components.jobCard.assignedToYou')}
              </Badge>
            )}
          </div>
          ...
          <div className="flex gap-2">
            ...
            {hasApplied && status === JOB_STATUS.OPEN && (
              <Badge
                variant="outline"
                className="rounded-none text-xs bg-primary/10"
              >
            ...
            {isOwner && status === JOB_STATUS.OPEN && applicantCount > 0 && (
              ...
              >
                {t('components.jobCard.viewApplicants', {
        ...
        <DialogContent className="rounded-none border-border">
          <DialogHeader>
            <DialogTitle className="font-sans">
              {t('components.jobCard.applyForJob')}
            </DialogTitle>
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="bg-muted/50 p-3 border border-border text-xs space-y-1">
              <p className="font-mono text-muted-foreground">
                {t('components.jobCard.budgetLabel')}:{' '}
      ...
      <Dialog open={showApplicantsModal} onOpenChange={setShowApplicantsModal}>
        <DialogContent className="rounded-none border-border max-w-md">
          <DialogHeader>
            <DialogTitle className="font-sans">
              {t('components.jobCard.jobApplicants')}
            </DialogTitle>
            <DialogDescription className="font-mono text-xs">
              {t('components.jobCard.applicantsForJob', {
                count: applicantCount,
                title,
              })}
            </DialogDescription>
          ...
          <div className="space-y-2 py-4 max-h-[300px] overflow-y-auto">
            {loadingApplicants ? (
              ...
              <div className="text-center py-8 text-muted-foreground">
                ...
                <p className="text-sm">
                  {t('components.jobCard.noApplicants')}
                </p>
              </div>
            )}
          </div>
