...
const CreateProject = () => {
  ...
  const { isConnected, refreshData } = useWallet()
  ...
  const handleSubmit = async e => {
    ...
    if (CONTRACTS.PACKAGE_ID === 'TO_BE_DEPLOYED') {
      toast.error(
        'Smart contracts not deployed yet! Please deploy contracts first.'
      )
      return
    }
    ...
    <div className="max-w-2xl mx-auto space-y-8">
      <div className="space-y-2">
        ...
        <p className="text-muted-foreground font-mono text-sm">
          Record your work on-chain. All submissions are subject to peer
          verification.
        </p>
      </div>
      ...
      <Card className="border-border bg-card/50">
        <CardContent className="pt-6">
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-2">
              <Label htmlFor="type" className="font-mono uppercase text-xs">
                {t('contribute.projectType')}
              </Label>
              <Select
                required
            ...
            <div className="space-y-2">
              <Label htmlFor="proof" className="font-mono uppercase text-xs">
                {t('contribute.proofLink')}
              </Label>
              <Input
                id="proof"
                type="url"
